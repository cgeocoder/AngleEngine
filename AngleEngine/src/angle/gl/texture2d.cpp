...
Texture2D::Texture2D(const std::string& _Path) : m_Path{ _Path } {
	...
	GL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE)); // GL_CLAMP_TO_EDGE
	GL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE)); // GL_REPEAT

	GL(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, // GL_RGBA8
		m_Width, m_Height, 0, GL_RGBA, GL_UNSIGNED_BYTE, m_Data));
